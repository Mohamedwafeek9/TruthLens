...
CFG = {
    "alpha":             0.65,   # smoothing أقل = استجابة أسرع
    "max_persons":       8,
    "track_dist":        120,
...
def keypoints_to_box(k, sx, sy):
    """
    بيحسب bounding box حقيقي من نقاط الوجه والكتفين.
    بيرجع (x1, y1, x2, y2) في إحداثيات الـ frame الأصلي.
    """
    # الوجه: nose(0), left_eye(1), right_eye(2), left_ear(3), right_ear(4)
    # الكتفين: left(5), right(6)
    ...
    w_pad = max(w_span * 0.5, 35)
    h_pad = max(h_span * 0.6, 40)

    x1 = int(min(xs) - w_pad)
    y1 = int(min(ys) - h_pad)        # فوق الرأس
    x2 = int(max(xs) + w_pad)
    y2 = int(max(ys) + h_pad * 1.8)  # تحت الكتفين
    ...
    try:
        ...
        while state["running"]:
            ...
            if use_picam:
                ...
                # FIX الألوان: auto-detect channels
                if array.shape[2] == 4:
                    frame = cv2.cvtColor(array, cv2.COLOR_RGBA2BGR)
                else:
            ...
            if pose_res.keypoints is not None:
                for k in pose_res.keypoints.data.cpu().numpy():
                    ...
                    if box is not None:
                        ...
                        label_y = max(y1 - 8, 16)
                        cv2.putText(frame, pid, (x1 + 4, label_y),
